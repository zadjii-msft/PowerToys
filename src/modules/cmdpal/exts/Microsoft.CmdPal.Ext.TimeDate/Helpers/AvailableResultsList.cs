...
internal static class AvailableResultsList
{
    ...
    internal static List<AvailableResult> GetList(bool? timeLongFormat = null, bool? dateLongFormat = null, DateTime? timestamp = null, CalendarWeekRule? firstWeekOfYear = null, DayOfWeek? firstDayOfWeek = null)
    {
        ...
        var timeExtended = timeLongFormat ?? SettingsManager.Instance.TimeWithSeconds;
        var dateExtended = dateLongFormat ?? SettingsManager.Instance.DateWithWeekday;
        var isSystemDateTime = timestamp == null;
        var dateTimeNow = timestamp ?? DateTime.Now;
        var dateTimeNowUtc = dateTimeNow.ToUniversalTime();
        var firstWeekRule = firstWeekOfYear ?? TimeAndDateHelper.GetCalendarWeekRule(SettingsManager.Instance.CalendarFirstWeekRule);
        var firstDayOfTheWeek = firstDayOfWeek ?? TimeAndDateHelper.GetFirstDayOfWeek(SettingsManager.Instance.FirstDayOfWeek);
