...
public static class InputInterpreter
{
    ...
    public static void InputSpaceInserter(ref string[] split)
    {
        ...
        var parseInputWithoutSpace = Regex.Split(split[0], Pattern);

        if (parseInputWithoutSpace.Length > 1)
        {
            var firstEntryRemoved = split.Skip(1).ToArray();
            var newSplit = new string[] { parseInputWithoutSpace[0], parseInputWithoutSpace[1] };
    ...
    public static void ShorthandFeetInchHandler(ref string[] split, CultureInfo culture)
    {
        ...
        if (split.Length == 3)
        {
            var shortsplit = RegexSplitter(split);
            ...
            switch (shortsplit.Length)
            {
                case 2:
                    ...
                    if (shortsplit[1] == "\'")
                    {
                        var newInput = new string[] { shortsplit[0], "foot", split[1], split[2] };
                        split = newInput;
                    }
                    else if (shortsplit[1] == "\"")
                    {
                        var newInput = new string[] { shortsplit[0], "inch", split[1], split[2] };
                        split = newInput;
                    }
                ...
                case 4:
                    ...
                    if (shortsplit[1] == "\'")
                    {
                        var isNegative = shortsplit[0].StartsWith('-');
                        if (isNegative)
                        {
                            shortsplit[0] = shortsplit[0].Remove(0, 1);
                        }

                        var isFeet = double.TryParse(shortsplit[0], NumberStyles.AllowDecimalPoint, culture, out var feet);
                        var isInches = double.TryParse(shortsplit[2], NumberStyles.AllowDecimalPoint, culture, out var inches);
                        ...
                        var convertedTotalInFeet = Length.FromFeetInches(feet, inches).Feet;
                        if (isNegative)
                        {
                            convertedTotalInFeet *= -1;
                        }

                        var newInput = new string[] { convertedTotalInFeet.ToString(culture), "foot", split[1], split[2] };
                        split = newInput;
                    }
    ...
    // TODO: Query object comes from Wox.Plugin - in most cases, Query.Search should just be treated like a string.
    // We need to figure out a wrapper for this.
    public static ConvertModel Parse(string query)
    {
        ...
        if (!double.TryParse(split[0], out var value))
        {
            return null;
        }
